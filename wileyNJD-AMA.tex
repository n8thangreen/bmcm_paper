...
Survival data are instrumental to Health Technology Assessment (HTA) evaluation \cite{Demiris2006, Jackson2010}..
Interventions that impact upon the time to events of interest, such as disease progression or death, form a high proportion of the appraisals by NICE \citep{Latimer2011}.
For instance, a new intervention may be expected to lengthen the amount of time a patient remains progression-free.
Further, multiple end-points for each patient may we considered in a single analysis.
...
This is in contrast to general survival curve summaries, such as the median time.
...
That is, the survival curve converges to a probability greater than zero and does not appear to decrease further.
...
How to principally and usefully do this for trials with multiple treatments and end-points is the subject of this paper.
...
% swap order so PFS is before OS
\begin{figure}[!HtH]
\centering
...
It assumes a mixture of two distinct groups so that the full set of model parameters consist of two separate groups of parameters $\bm\omega = (\bm\theta_b, \bm\theta_u)$.
...
\subsubsection{Issues in an HTA context}
There are several issues with existing approaches to cure modelling in an HTA context in particular.
...
The censoring times may be too premature to adequately characterise a survival model for a single event-type but by taking advantage of the set of event-types together this may be alleviated to give better fits and extrapolation out to the full lifetimes of patients which is particularly important for health economics modelling in order to gauge the full cost and effect of an intervention.
