...
We present a general Bayesian hierarchical framework that can handle this complexity by exploiting the similarity of cure fractions between outcomes;
accounting for the correlation between them and improving the extrapolation beyond the observed data.
Assuming exchangeability between cure fractions facilitates the borrowing of information between outcome.
...
Survival data are instrumental to Health Technology Assessment (HTA) evaluation.
Interventions that impact upon the time to some event of interest, such as disease progression or death, form a high proportion of the appraisals by NICE \citep{Latimer2011}.
For instance, a new intervention may be expected to lengthen the amount of time a patient remains progression-free.

% extrapolation
In order to quantify accurately the health and economic benefits of a new intervention using survival data it is standard to estimate the {\it mean survival time}.
This is in contrast to usual survival curve summaries, such as the median time.
In order to calculate the mean survival time we require the full survival curve, i.e. over a patient's lifetime, but available data often only cover a limited time frame and are subject to a high degree of censoring. Thus, it is necessary to extrapolate the observed survival curve to a longer time horizon.
A parametric approach to the survival analysis is usually followed to achieve this, as recommended by a highly influential NICE Decision Support Unit (DSU) Technical Support Document (TSD) \citep{Latimer2011}.

% plateau
In the past decade, the development of potentially highly successful immuno-oncology treatments, and better diagnosis and screening has lead to cancer patients with improved survival outcomes and more long-term survival (LTS) patients comparable to the general population.
...
results have indicated that survival curves "plateau" for a considerable proportion of LTS patients in the limited range of the available survival curve [ref].
...
However, decisions are often made on an early data cut when the amount of information available to inform the decision is relatively small.
Specifically, the survival plateau indicating a proportion of LTS may not have been reached at the time of an earlier data cut.
In order to produce a complete survival curve to use in an HTA evaluation we would need to extrapolate the survival curve to the sustained plateau. 
% This may be instrumental into justifying how we want to link up PFS and OS.
How to principally and usefully do this is the subject of this paper.
...
The population that is not cured is subject both to background mortality and to additional mortality from their disease, estimated using a survival model.
\cite{Amico2018}
...
% issues
There are several issues with existing approaches to cure modelling.
...
The censoring times may be too premature to adequately characterise a survival model for a single event-type but by taking advantage of the set of event-types together this may be alleviated to give better survival model estimates.

% extrapolation
Lastly, extrapolation out to the full lifetimes of patients is particularly important for health economics modelling in order to gauge the full cost and effect of an intervention.
This has not yet been a focus of cure modelling work.

% our innovative proposal
Building on the existing literature, the main contributions of this paper are to address all of these issues.
We will present a Bayesian hierarchical cure fraction model for multiple outcomes.
We will show that this has the double advantage of borrowing information between outcomes
...
Survival analysis generally is a core approach used in health economics.
For a non-cure model survival analysis in health economics see \cite{Demiris2006, Jackson2010}.
...
Correlation between random effects in the uncured survival and cure fraction has also been investigated using a bivariate Normal distribution \cite{Lai2008}.
However, to our knowledge there has been no Bayesian fully-parametric mixture cure model with a multi-level modelling structure for outcomes.
...
The data source for the analysis consists of the patient-level data from the phase 3 CheckMate 067 trial\cite{Larkin2019}.
...
where $\bm{t}_i = (\bm{t}_{1i}, \bm{t}_{2i})$, $\bm{\delta}_i = (\bm{\delta}_{1i}, \bm\delta_{2i})$.

In general, we model the observed times using a parametric sampling distribution $t_{ji} \sim p(t_{ji} \mid \bm\theta_j)$ as a function of a set of model parameters $\bm\theta_j = (\bm\alpha_j(\bm x), \mu_j(\bm x))$. 
...
Whilst it is possible for both $\mu$ and $\alpha$ to explicitly depend on the covariates $\bm x$, usually the formulation is simplified to assume that these only affect directly the location parameter.
Since $t > 0$, we typically use a generalized linear formulation
$$
...
The standard mixture cure model (MCM) (sometimes called a long-term survival model), which is the basis of the proposed model, assumes a mixture of two distinct groups so that the full set of model parameters consist of two separate groups of parameters $\bm\omega = (\bm\theta_b, \bm\theta_u)$.
...
Denote the WHO estimates for individual $i$ used for the background survival in (\ref{eqn:mcm}) as
$S_b(t_i \mid \bm\theta_b, \bm{x}_i) = \hat{S}_i$.

Gamma process? \cite{Demiris2006}
...
Follow-up time can be relatively short for some of the outcomes and so there is missing data due to censoring.
...
Bivariate Normal placed on the OS and PFS rates could be one option for modelling the connection\cite{Tan2018}.
...
%
\subsubsection{Representing the cure fraction when there are multiple outcomes}
...
\subsubsection{Prior specification for motivating example}
...
The hierarchical and separate models are fit for Exponential, Weibull, Gompertz, Log-Normal, Log-Logistic to the OS and PFS event times.

\subsection{Overall results}

The global cure fraction posterior is wide which is understandable since there are only two exchangeable points providing limited information (OS and PFS).
However, despite this there is influence pulling the central location away from the prior downwards for {\it ipilimumab} and upwards for the dual treatments.

% plateau time
The mixed survival curves plateau to the background mortality survival from when the uncured survival curves meet the $x$-axis so since this is user-supplied then there is no additional insight.
This is particularly early for PFS and the log-Normal distribution.
