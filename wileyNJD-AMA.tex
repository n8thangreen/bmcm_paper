...
% how survival data are instrumental to HTA evaluation (you can look at the survHE manual/paper intro)
% sometimes, especially since the development of potentially highly successful immuno-oncology treatment, you tend to see a plateau in the **limited** range of the available survival curve/data
% bring these two together by highlighting the fact that often you have consecutive data cuts (with increasingly better information/lower censoring), but decisions are often made on a very early cut. This may be instrumental into justifying how we want to link up PFS and OS.
% we can bring about the motivating example and the plan of the chapter.



% set the scene
% cancer trials have two groups
Better diagnosis and screening has lead to more cancer patients with long-term survival (LTS) equivalent to the general population.
...
results have indicated that survival curves "plateau" for a considerable proportion of LTS patients [ref].
This proportion of patients is often called the {\it cure fraction}. 
The term "cured" here means that a patient has responded to treatment and is a LTS.

% what are cure models
The use of cure models is a special type of survival analysis where this cure fraction is accounted for.
Cure models split patients in to two groups (cured or not) and individuals are subject to one or two sources of risk.
...
% why are cure models important
Cure models are important because ... if you don't account for a mixture then give the wrong results

Specifically, when applied in health economics modelling, cure models are important because the results of lifetime survival are used in cost-effectiveness analyses which influence policy decisions.
Incorrect modelling will misinform the decision-makers.

% brief review of existing applications

{\it BMC to give background...}
...
% censoring
Secondly, time to event data are typically right-censored times due to end to follow-up.
...
% extrapolation
Lastly, extrapolation out to the full life times of patients is particularly important for health economics modelling in order to gauge the full effect of an intervention.
This has not been a focus of cure modelling work.
...
The data source for the analysis consists of the patient-level data from the CheckMate 067 trial\cite{Larkin2019}.
...
Denote the WHO estimates for individual $i$ and outcome $j$ as
$\hat{p}_{ij}, \hat{S}_i, \hat{h}_i$ for the density,
survival and hazard respectively.

This gives
\begin{equation*}
p(t_{ij} \mid \bm\theta, x_{ij}) = \hat{S}_i \hat{h}_i^{\delta_{ij}} \left[\pi(x_{ij}) + (1 - \pi(x_{ij})) S_u(t_{ij} \mid \theta_u, \boldsymbol{x}_{ij}) h_u(t_{ij} \mid \boldsymbol{x}_{ij})^{\delta_{ij}} \right]
\end{equation*}
